# ACMS Match Control — Admin Portal v4.53 — Normalized Wildcard PQ
...
## v4.53 — Fair wildcard normalization for three-pool divisions
For divisions with **3 pools/brackets**, semifinal qualification now uses this ACMS-specific wildcard rule:

> **Each Pool #1 automatically qualifies. The wildcard is selected from the Pool #2 teams using Normalized PQ only. Win Rate is not used for wildcard comparison.**
...
- The portal then recalculates wildcard-only **PE, PC and PQ = PE ÷ (PE + PC)** from the retained matches.
- Highest **Normalized PQ** becomes the wildcard.
- If two or more Pool #2 candidates have exactly the same Normalized PQ, automatic selection stops and a Tournament Director decision is required.
- Official standings, official W/L records, official PE/PC and official PQ are never modified by this normalization.
- The Playoffs qualification panel displays the official PQ, the excluded opponent/result, and the resulting Normalized PQ so the calculation can be audited during the event.
...
- Wildcard comparison becomes **PE 26, PC 20, Normalized PQ .5652** across three counted matches.
