# ACMS Admin v4.61 — Side Panel Style Authority

## v4.61 — Side panel customization cleanup
- Rebuilt the **Side Panel Style** settings layout as a full-width editor with a live preview.
- Solid and Gradient modes are clearly separated and responsive on desktop/mobile.
- Gradient mode exposes Primary Color, Secondary Color, and Direction controls; Solid mode hides the unnecessary secondary controls.
- Added a visible hex value beside each color picker.
- Consolidated sidebar rendering around one authoritative CSS variable: `--acms-sidebar-background`.
- Legacy Navy/Sky, light-mode, dark-mode, and mobile sidebar rules now resolve to that same variable instead of repainting the navigation with hard-coded colors.
- The saved sidebar style is loaded from localStorage in the document head before the main CSS renders, preventing a default-color flash before the custom style appears.
- A final authority rule at the end of the head protects the selected sidebar style from later legacy CSS patches.
- v4.60 Manual + Bulk Roster Intake and all tournament logic are retained.

# ACMS Admin v4.60 — Manual + Bulk Roster Intake

## v4.60 — Registration / roster intake
- Registrations now exposes **Manual Add**, **Bulk Import**, and **Import Template** controls.
- Manual Add supports Team Code, Hospital Code, optional Hospital Name, Division, Bracket A–D, both player names, and initial check-in status.
- Bulk Import accepts CSV, TSV, or rows pasted directly from Excel. For `.xlsx/.xls`, copy/paste the rows or save the sheet as CSV.
- Required columns: `team_code`, `hospital`, `division`, `bracket`, `player_1`, `player_2`.
- Optional columns: `hospital_name`, `checked_in`.
- Import modes: **Append new teams only**, **Add new + update existing**, and **Replace roster safely**.
- Validation blocks duplicate team codes, duplicate players, invalid brackets, restricted unknown divisions/hospitals, and division/bracket changes on teams already linked to match records.
- Safe Replace is blocked if an omitted team code is still referenced by any match record.
- Check-in handling can preserve existing attendance, use the `checked_in` column, force all unchecked, or force all checked.
- Bulk import never creates, deletes, or moves match records automatically.
- Imported roster data feeds the existing Check-In, Match Control, Standings, qualification, Player Search, and public Player Portal sync.

## v4.59 retained
- Admin logo and header name can be customized in Settings.
- Side panel can use a custom solid color or gradient with selectable direction.

# ACMS Admin v4.57 — 2/3/4-Bracket Auto Qualification
...
# ACMS Match Control — Admin Portal v4.57 — 2/3/4-Bracket Auto Qualification

Production Admin repository package for GitHub Pages.

## Production URLs
- Admin: https://qourts.github.io/ACMS/
- Player: https://qourts.github.io/ACMSplayer/
...
## v4.56 — Clean root GitHub Pages deployment
- The Admin application is now the repository root **`index.html`**.
- The old redirect `index.html → admin.html` has been removed.
- `admin.html` is no longer required or included in the deployment package.
- Production Admin URL is now **https://qourts.github.io/ACMS/**.
- Production Player URL is **https://qourts.github.io/ACMSplayer/**.
- Admin → Player links and schedule-appearance bridge targets now use the new Player URL.
- All v4.55 Navy/Sky first-paint protections and normalized Win Rate → normalized PQ wildcard logic are retained.
- Existing browser tournament data remains on the same GitHub Pages origin (`qourts.github.io`), so changing the page path from `/ACMS/admin.html` to `/ACMS/` does not itself clear localStorage.
...
## v4.57 — Four-bracket semifinal qualification

Automatic qualification now supports divisions with 2, 3, or 4 preliminary brackets.

- **2 brackets:** Top 2 per bracket; A1 vs B2 and B1 vs A2.
- **3 brackets:** Each bracket #1 + best #2 wildcard; wildcard comparison remains normalized match count → Normalized Win Rate → Normalized PQ.
- **4 brackets:** Each bracket #1 qualifies. The four bracket winners are seeded by Win Rate → PQ → lowest PC/Game → highest PE/Game → Tournament Director draw if still exactly tied. Semifinals are Seed #1 vs #4 and Seed #2 vs #3.
- Existing zero-loss Twice-to-Beat logic still applies after semifinal participants are seeded.
